...
//            var country = scountry_et.text.toString().trim()
            ...
            if (id != "" && name != "" && programme != null ) {
                var student = Student(id, name, programme.id, programme.name)
...
//        scountry_et.text.clear()
    ...
    private fun retrieveProgrammes(){
        ...
        programmeCollection.retrieveData(programmes as ArrayList<ModelBase>, Programme::class.java) {
    ...
    private fun retrieveStudents() {
        ...
        studentCollection.retrieveData(students as ArrayList<ModelBase>, Student::class.java){
    ...
    private fun writeStudent(student: Student) {
        val studentCollection = fb.collection(studentsPath)
        studentCollection.realTimeUpdate(students as ArrayList<ModelBase>, Student::class.java){
            studentAdapter.notifyDataSetChanged()
...
//               val date = (if (week == 1) it.startDate?.toDate() else DateUtil.addDays(it.startDate ?: Timestamp(Date()),7) ) ?: Date()
        ...
        val batch = fb.batch()
        attendances.forEach {
            val document = attendanceCollection.document()
